...
function http_post_request_with_all_response($url, array $params, $headers = [], $contenntType = '', $setopt = [])
{
    $formatParams = '';
    if (!$contenntType) {
        $formatParams = json_encode($params, JSON_UNESCAPED_UNICODE);
        $contenntType = 'application/json;charset=utf-8';
    } else {
        foreach ($params as $key => $value) {
            $formatParams .= $key . '=' . $value;
        }
    }
    $headers[] = "Content-Type:" . $contenntType;

    $curl = curl_init();
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($curl, CURLOPT_URL, $url);
    curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'POST');
    curl_setopt($curl, CURLOPT_POSTFIELDS, $formatParams);
    ...
    curl_close($curl);
    return ['statusCode' => $httpCode, 'responseData' => $outPut];
}
 ...
 * http get请求
 */
function http_get_request($url, array $params)
{
